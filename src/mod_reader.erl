...
login_parse_packet(Socket, Client) ->
    ...
    receive
        %% flash安全沙箱
        {inet_async, Socket, Ref, {ok, ?FL_POLICY_REQ}} -> %% 短链接模式
                ...
                true ->
                    ...
                    receive
                        {inet_async, Socket, Ref1, {ok, Binary}} ->
                            case routing(Cmd, Binary) of %% 解压数据
                                %% 先验证登陆
                                {ok, login, Data} ->
...
do_parse_packet(Socket, Client) ->
    io:format("~p ~p Args=~w~n", [?MODULE, ?LINE, [Socket, Client]]),
    Ref = async_recv(Socket, ?HEADER_LENGTH, ?HEART_TIMEOUT),
    receive
        {inet_async, Socket, Ref, {ok, <<Len:16, Cmd:16>>}} ->
            BodyLen = Len - ?HEADER_LENGTH,
        ...
        %%用户断开连接或出错
        Other ->
            io:format("~p ~p Other=~w~n", [?MODULE, ?LINE, Other]),
...
%% 接受信息
%% prim_inet一个更重要的工作，就是将port层次基于消息的异步交互机制，通过在发送消息后立即receive结果，
%% 挂起当前process，变成了阻塞的同步交互机制。
async_recv(Sock, Length, Timeout) when is_port(Sock) ->
    case prim_inet:async_recv(Sock, Length, Timeout) of %% 异步接收消息
