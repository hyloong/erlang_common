...
start_link() ->
    case gen_event:start_link({local, common_alarm_handler}) of
        {ok, Pid} ->
            %% execute alarm_handler:init()
            gen_event:add_handler(common_alarm_handler, common_alarm_handler, []),
            {ok, Pid};
        Error -> Error
    end.
...
%%-----------------------------------------------------------------
set_alarm(Alarm) ->
    gen_event:notify(common_alarm_handler, {set_alarm, Alarm}).
...
%%-----------------------------------------------------------------
clear_alarm(AlarmId) ->
    gen_event:notify(common_alarm_handler, {clear_alarm, AlarmId}).
...
%%-----------------------------------------------------------------
get_alarms() ->
    gen_event:call(common_alarm_handler, alarm_handler, get_alarms).

add_alarm_handler(Module) when is_atom(Module) ->
    gen_event:add_handler(common_alarm_handler, Module, []).

add_alarm_handler(Module, Args) when is_atom(Module) ->
    gen_event:add_handler(common_alarm_handler, Module, Args).

delete_alarm_handler(Module) when is_atom(Module) ->
    gen_event:delete_handler(common_alarm_handler, Module, []).
