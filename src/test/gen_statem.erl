...
-spec start_link(Module :: module(), Args :: term(), Opts :: [start_opt()]) ->
                        start_ret().
...
-spec call(
        ...
        Request :: term(),
        Timeout :: timeout()
                 | {'clean_timeout',T :: timeout()} %% 注意描述
                 | {'dirty_timeout',T :: timeout()}) %% 传送到代理进程，影响较小
          -> Reply :: term().
%% Timeout，caller应该catch表达式，避免一个较晚的回复，会创建一个代理进程来执行这个call的操作。
%% 一个迟的回复发送到代理进程，会被丢弃。所以最好用Timeout==infinity
...
sys_debug(Debug, #{name := Name}, State, Entry) ->
    case Debug of
        [] ->
            Debug;
        _ ->
            sys:handle_debug(Debug, fun print_event/3, {Name,State}, Entry)
    end.
...
parse_actions(Debug, S, State, Actions) ->
    ...
    Postpone = false,
    NextEventsR = [],
    parse_actions(Debug, S, State, listify(Actions), Hibernate, TimeoutsR, Postpone, NextEventsR).
%%
parse_actions(Debug, _S, _State, [], Hibernate, TimeoutsR, Postpone, NextEventsR) ->
    {ok,Debug,Hibernate,TimeoutsR,Postpone,NextEventsR};
parse_actions(Debug, S, State, [Action|Actions], Hibernate, TimeoutsR, Postpone, NextEventsR) ->
    case Action of
        %% Actual actions
        {reply,From,Reply} -> %% 返回
            case from(From) of
                true ->
                    ...
                    parse_actions(NewDebug, S, State, Actions, Hibernate, TimeoutsR, Postpone, NextEventsR);
                false ->
                    {error,
        ...
        {hibernate,NewHibernate} when is_boolean(NewHibernate) ->
            parse_actions(Debug, S, State, Actions, NewHibernate, TimeoutsR, Postpone, NextEventsR);
        hibernate ->
            NewHibernate = true,
            parse_actions(Debug, S, State, Actions, NewHibernate, TimeoutsR, Postpone, NextEventsR);
        %%
        {postpone,NewPostpone}
          when is_boolean(NewPostpone), Postpone =/= forbidden ->
            parse_actions(Debug, S, State, Actions, Hibernate, TimeoutsR, NewPostpone, NextEventsR);
        ...
        {next_event,Type,Content} ->
            case event_type(Type) of
                true when NextEventsR =/= forbidden ->
                    NewDebug = sys_debug(Debug, S, State, {in, {Type, Content}}),
                    parse_actions(NewDebug, S, State, Actions, Hibernate, TimeoutsR, Postpone, [{Type, Content}|NextEventsR]);
                _ ->
                    {error, {bad_action_from_state_function,Action}, ?STACKTRACE()}
            end;
        %%
        {{timeout,_},_,_} = Timeout ->
            parse_actions_timeout(Debug, S, State, Actions, Hibernate, TimeoutsR, Postpone, NextEventsR, Timeout);
    ...
    case Timeout of
        {TimerType, Time, TimerMsg, TimerOpts} ->
            case validate_timer_args(Time, listify(TimerOpts)) of
                true ->
                    parse_actions(Debug, S, State, Actions, Hibernate, [Timeout|TimeoutsR], Postpone, NextEventsR);
                false ->
